...
from SkyButler.modules.helper_funcs.handlers import CMD_STARTERS
from SkyButler.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    from SkyButler.modules.helper_funcs.chat_status import user_admin, is_user_admin
    from telegram.ext.dispatcher import run_async

    from SkyButler.modules.sql import disable_sql as sql
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text(sbt(chat_id, "Disabled the use of `{}`").format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text(sbt(chat_id, "That command can't be disabled"))

        else:
            update.effective_message.reply_text(sbt(chat_id, "What should I disable?"))
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text(sbt(chat_id, "Enabled the use of `{}`").format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text(sbt(chat_id, "Is that even disabled?"))

        else:
            update.effective_message.reply_text(sbt(chat_id, "What should I enable?"))
    ...
    def list_cmds(bot: Bot, update: Update):
        ...
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text(sbt(chat_id, "The following commands are toggleable:\n{}").format(result),
                                                parse_mode=ParseMode.MARKDOWN)
        else:
            update.effective_message.reply_text(sbt(chat_id, "No commands can be disabled."))
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return sbt(chat_id, "No commands are disabled!")

        result = ""
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return sbt(chat_id, "The following commands are currently restricted:\n{}").format(result)
