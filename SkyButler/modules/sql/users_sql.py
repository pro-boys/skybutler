...
from SkyButler.modules.sql import BASE, SESSION
...
class Chats(BASE):
    ...
    safemode_status = Column(Integer, default=0)
...
def get_chat_members(chat_id: object) -> object:
    try:
...
def set_safemode(chat_id, safemode_status):
    with INSERTION_LOCK:
        curr = SESSION.query(Chats).get(str(chat_id))
        if not curr:
            curr = Chats(str(chat_id))
        curr.safemode_status = int(safemode_status)

        SESSION.add(curr)
        SESSION.commit()


def get_safemode(chat_id):
    safe = SESSION.query(Chats).get(str(chat_id))
    SESSION.close()

    if safe:
        return safe.safemode_status
