...
import SkyButler.modules.sql.blacklist_sql as sql
from SkyButler import dispatcher, LOGGER
from SkyButler.modules.disable import DisableAbleCommandHandler
from SkyButler.modules.helper_funcs.chat_status import user_admin, user_not_admin
...
def blacklist(bot: Bot, update: Update, args: List[str]):
    ...
    for text in split_text:
        if text == BASE_BLACKLIST_STRING:
            msg.reply_text(sbt(chat_id, "There are no blacklisted messages here!"))
            return
...
def add_blacklist(bot: Bot, update: Update):
    ...
    if len(words) > 1:
        ...
        if len(to_blacklist) == 1:
            msg.reply_text(sbt(chat_id, "Added <code>{}</code> to the blacklist!").format(html.escape(to_blacklist[0])),
                           parse_mode=ParseMode.HTML)

        else:
            msg.reply_text(
                sbt(chat_id, "Added <code>{}</code> triggers to the blacklist.").format(len(to_blacklist)), parse_mode=ParseMode.HTML)

    else:
        msg.reply_text(sbt(chat_id, "Tell me which words you would like to add to the blacklist."))
...
def unblacklist(bot: Bot, update: Update):
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                msg.reply_text(sbt(chat_id, "Removed <code>{}</code> from the blacklist!").format(html.escape(to_unblacklist[0])),
                               parse_mode=ParseMode.HTML)
            else:
                msg.reply_text(sbt(chat_id, "This isn't a blacklisted trigger...!"))

        elif successful == len(to_unblacklist):
            msg.reply_text(
                sbt(chat_id, "Removed <code>{}</code> triggers from the blacklist.").format(
                    successful), parse_mode=ParseMode.HTML)

        elif not successful:
            msg.reply_text(
                sbt(chat_id, "None of these triggers exist, so they weren't removed.").format(
                    successful, len(to_unblacklist) - successful), parse_mode=ParseMode.HTML)

        else:
            msg.reply_text(
                sbt(chat_id, "Removed <code>{}</code> triggers from the blacklist. {} did not exist, "
                "so were not removed.").format(successful, len(to_unblacklist) - successful),
                parse_mode=ParseMode.HTML)
    else:
        msg.reply_text(sbt(chat_id, "Tell me which words you would like to remove from the blacklist."))
...
def __chat_settings__(chat_id, user_id):
    ...
    return sbt(chat_id, "There are {} blacklisted words.").format(blacklisted)
