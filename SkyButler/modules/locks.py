...
import SkyButler.modules.sql.locks_sql as sql
from SkyButler import dispatcher, SUDO_USERS, LOGGER
from SkyButler.modules.disable import DisableAbleCommandHandler
from SkyButler.modules.helper_funcs.chat_status import can_delete, is_user_admin, user_not_admin, user_admin, \
...
def add_whitelist(bot: Bot, update: Update):
    ...
    for url in entities.values():
        if sql.add_whitelist(chat.id, url):
            added.append(url)
    if added:
        message.reply_text(sbt(chat_id, "Added to whitelist:\n- ")+'\n- '.join(added))
        return "<b>{}:</b>" \
               "\n#WHITELIST" \
               "\n<b>Admin:</b> {}" \
        ...
        message.reply_text(sbt(chat_id, "No URLs were added to the whitelist"))
        return ""
...
def remove_whitelist(bot: Bot, update: Update):
    ...
    for url in entities.values():
        if sql.remove_whitelist(chat.id, url):
            removed.append(url)
    if removed:
        message.reply_text(sbt(chat_id, "Removed from whitelist:\n<pre>- {}</pre>").format(html.escape('\n- '.join(removed))),
        ...
        message.reply_text(sbt(chat_id, "Could not remove URL from whitelist or URL not found."))
        return ""
...
def list_whitelist(bot: Bot, update: Update):
    ...
    if not all_whitelisted:
        message.reply_text(sbt(chat_id, "No URLs are whitelisted here!"))
        return
    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text(sbt(chat_id, "Locked {} messages for all non-admins!").format(args[0]))
                ...
                message.reply_text(sbt(chat_id, "Locked {} for all non-admins!").format(args[0]))
                return "<b>{}:</b>" \
                       "\n#LOCK" \
                       "\n<b>Admin:</b> {}" \
                       "\nLocked <code>{}</code>.".format(html.escape(chat.title),
                ...
                message.reply_text(sbt(chat_id, "What are you trying to lock...? Try /locktypes for the list of lockables"))

    else:
        message.reply_text(sbt(chat_id, "I'm not an administrator, or haven't got delete rights."))
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text(sbt(chat_id, "Unlocked {} for everyone!").format(args[0]))
                return "<b>{}:</b>" \
                       "\n#UNLOCK" \
                       "\n<b>Admin:</b> {}" \
                ...
                message.reply_text(sbt(chat_id, "What are you trying to unlock...? Try /locktypes for the list of lockables"))

        else:
            bot.sendMessage(chat.id, sbt(chat_id, "What are you trying to unlock...?"))
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text(sbt(chat_id, "I see a bot, and I've been told to stop them joining... "
                                               "but I'm not admin!"))
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text(sbt(chat_id, "Only admins are allowed to add bots to this chat! Get outta here."))
            else:
...
def build_lock_message(chat_id):
    ...
    if not (locks or restr):
        res = sbt(chat_id, "There are no current locks in this chat.")
    else:
