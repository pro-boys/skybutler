...
import SkyButler.modules.sql.rules_sql as sql
...
def send_rules(update, chat_id, from_pm=False):
    ...
    if from_pm and rules:
        ...
        update.effective_message.reply_text(sbt(chat_id, "Contact me in PM to get this group's rules."),
                                            reply_markup=InlineKeyboardMarkup(
                                                [[InlineKeyboardButton(text="Rules",
                                                                       url="t.me/{}?start={}".format(bot.username,
                                                                                                     chat_id))]]))
    else:
        update.effective_message.reply_text(sbt(chat_id, "The group admins haven't set any rules for this chat yet."))
...
def set_rules(bot: Bot, update: Update):
    ...
    if len(args) == 2:
        ...
        update.effective_message.reply_text(sbt(chat_id, "Successfully set rules for this group."))
...
def clear_rules(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text(sbt(chat_id, "Successfully cleared rules!"))
...
#    "This chat has had it's rules set: `{}`".format(bool(sql.get_rules(chat_id)))
