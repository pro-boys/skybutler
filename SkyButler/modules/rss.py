...
from SkyButler import dispatcher, updater
from SkyButler.modules.helper_funcs.chat_status import user_admin
from SkyButler.modules.sql import rss_sql as sql
...
def show_url(bot, update, args):
    ...
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            update.effective_message.reply_text(sbt(update.effective_chat.id, "This link is not an RSS Feed link"))
    else:
        update.effective_message.reply_text(sbt(update.effective_chat.id, "URL missing"))
...
def add_url(bot, update, args):
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            if row:
                update.effective_message.reply_text(sbt(update.effective_chat.id, "This URL has already been added"))
            else:
                sql.add_url(tg_chat_id, tg_feed_link, tg_old_entry_link)

                update.effective_message.reply_text(sbt(update.effective_chat.id, "Added URL to subscription"))
        else:
            update.effective_message.reply_text(sbt(update.effective_chat.id, "This link is not an RSS Feed link"))
    else:
        update.effective_message.reply_text(sbt(update.effective_chat.id, "URL missing"))
    ...
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            if user_data:
                ...
                update.effective_message.reply_text(sbt(update.effective_chat.id, "Removed URL from subscription"))
            else:
                update.effective_message.reply_text(sbt(update.effective_chat.id, "You haven't subscribed to this URL yet"))
        else:
            update.effective_message.reply_text(sbt(update.effective_chat.id, "This link is not an RSS Feed link"))
    else:
        update.effective_message.reply_text(sbt(update.effective_chat.id, "URL missing"))
