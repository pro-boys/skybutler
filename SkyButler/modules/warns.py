...
from SkyButler.modules.helper_funcs.chat_status import is_user_admin, bot_admin, user_admin, \
    can_restrict
from SkyButler.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
from SkyButler.modules.helper_funcs.filters import CustomFilters
from SkyButler.modules.helper_funcs.misc import split_message
from SkyButler.modules.helper_funcs.string_handling import split_quotes
from SkyButler.modules.log_channel import loggable
from SkyButler.modules.sql import warns_sql as sql
...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    if is_user_admin(chat, user.id):
        message.reply_text(sbt(chat.id, "Damn admins, can't even be warned!"))
        return ""

    if warner:
        warner_tag = mention_html(warner.id, warner.first_name)
    else:
        warner_tag = sbt(chat.id, "Automated warn filter.")
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = sbt(chat.id, "{} warnings, {} has been kicked!").format(limit, mention_html(user.id, user.first_name))

        else:  # ban
            chat.kick_member(user.id)
            reply = sbt(chat.id, "{} warnings, {} has been banned!").format(limit, mention_html(user.id, user.first_name))
        ...
        reply = sbt(chat.id, "{} has {}/{} warnings... watch out!").format(mention_html(user.id, user.first_name), num_warns,
                                                             limit)
        if reason:
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if not is_user_admin(chat, int(user.id)):
            query.answer(text=sbt(chat_id, "You are not authorized to remove this warn! Only administrators may remove warns."), show_alert=True)
            return ""
        res = sql.remove_warn(user_id, chat.id)
        if res:
            update.effective_message.edit_text(
                sbt(chat_id, "Warn removed by {}.").format(mention_html(user.id, user.first_name)),
            ...
            update.effective_message.edit_text(
                sbt(chat_id, "User has already has no warns.").format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warn_user(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text(sbt(chat_id, "No user was designated!"))
    return ""
...
def reset_warns(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text(sbt(chat_id, "Warnings have been reset!"))
        ...
        message.reply_text(sbt(chat_id, "No user has been designated!"))
    return ""
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            ...
            update.effective_message.reply_text(
                sbt(chat_id, "User has {}/{} warnings, but no reasons for any of them.").format(num_warns, limit))
    else:
        update.effective_message.reply_text(sbt(chat_id, "This user hasn't got any warnings!"))
...
def add_warn_filter(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text(sbt(chat_id, "Warn handler added for '{}'!").format(keyword))
...
def remove_warn_filter(bot: Bot, update: Update):
    ...
    if not chat_filters:
        msg.reply_text(sbt(chat_id, "No warning filters are active here!"))
        return

    for filt in chat_filters:
        if filt == to_remove:
            sql.remove_warn_filter(chat.id, to_remove)
            msg.reply_text(sbt(chat_id, "Yep, I'll stop warning people for that."))
            raise DispatcherHandlerStop

    msg.reply_text(sbt(chat_id, "That's not a current warning filter - run /warnlist for all active warning filters."))
...
def list_warn_filters(bot: Bot, update: Update):
    ...
    if not all_handlers:
        update.effective_message.reply_text(sbt(chat_id, "No warning filters are active here!"))
        return
...
def set_warn_limit(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                msg.reply_text(sbt(chat_id, "The minimum warn limit is 3!"))
            else:
                sql.set_warn_limit(chat.id, int(args[0]))
                msg.reply_text(sbt(chat_id, "Updated the warn limit to {}").format(args[0]))
                return "<b>{}:</b>" \
                       "\n#SET_WARN_LIMIT" \
                       "\n<b>Admin:</b> {}" \
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            msg.reply_text(sbt(chat_id, "Too many warns will now result in a ban!"))
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
            ...
            msg.reply_text(sbt(chat_id, "Too many warns will now result in a kick! Users will be able to join again after."))
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
            ...
            msg.reply_text(sbt(chat_id, "I only understand on/yes/no/off!"))
    else:
        limit, soft_warn = sql.get_warn_setting(chat.id)
        if soft_warn:
            msg.reply_text(sbt(chat_id, "Warns are currently set to *kick* users when they exceed the limits."),
                           parse_mode=ParseMode.MARKDOWN)
        else:
            msg.reply_text(sbt(chat_id, "Warns are currently set to *ban* users when they exceed the limits."),
                           parse_mode=ParseMode.MARKDOWN)
    return ""
...
def __chat_settings__(chat_id, user_id):
    ...
    return sbt(chat_id, "This chat has `{}` warn filters. It takes `{}` warns before the user gets *{}*.").format(num_warn_filters, limit, "kicked" if soft_warn else "banned")
