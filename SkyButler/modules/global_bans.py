...
import SkyButler.modules.sql.global_bans_sql as sql
from SkyButler import dispatcher, OWNER_ID, SUDO_USERS, STRICT_GBAN
from SkyButler.modules.helper_funcs.chat_status import bot_admin, is_user_admin
...
from SkyButler.modules.helper_funcs.misc import send_to_list
from SkyButler.modules.sql.users_sql import get_all_chats
...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text("*Blows dust off banhammer*")
...
@run_async
def master_gban(bot: Bot, update: Update, args: List[str]):
    message = update.effective_message  # type: Optional[Message]
    ...
    if int(user_id) in SUDO_USERS:
        message.reply_text ("You are gbanning a sudo user but anyway you are the master so i'll gban him")
...
def gbanstat(bot: Bot, update: Update, args: List[str]):
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text("I've enabled gbans in this group.")
...
MASTER_GBAN_HANDLER = CommandHandler("master_gban", master_gban, pass_args=True,
...
GBAN_STATUS = CommandHandler("gbanstat", gbanstat, pass_args=True, filters=CustomFilters.sudo_filter)

GBAN_ENFORCER = MessageHandler(Filters.all & Filters.group, enforce_gban)

dispatcher.add_handler(GBAN_HANDLER)
dispatcher.add_handler(MASTER_GBAN_HANDLER)
